package scalatron.botwar.botPlugin.gard
    ...
    */
    opcode match {
      case "React" => {
        ...
        val params2 = rest.split(',')                    // = Array("key=value", "key=value", ...)
        val strPairs = params2.map(s => s.split('='))    // = Array( Array("key","value"), Array("key","value"), ..)
        val kvPairs = strPairs.map(a => (a(0),a(1)))    // = Array( ("key","value"), ("key","value"), ..)
        val paramMap = kvPairs.toMap

        val bot: BotImpl = BotImpl(params)
        val generation = bot.generation
        val view = View(paramMap.get("view").get)

        val offsetPlant = view.offsetToNearest(Cell.GoodPlant)
        val offsetWall = view.offsetToNearest(Cell.Wall)
        val offsetBeast = view.offsetToNearest(Cell.GoodBeast)
        val offsetBadAss = view.offsetToNearest(Cell.BadBeast)
        val offsetEnemy = view.offsetToNearest(Cell.Enemy)
        val scareLevel = 3

        val slavemode = offsetBadAss match {
          case Some(xy) if (xy.y < 1 && xy.y > -1)||(xy.x < 1 && xy.x > -1) => true
          case _ => {
            offsetEnemy match {
              case Some(xy) if (xy.y < 1 && xy.y > -1)||(xy.x < 1 && xy.x > -1) => true
case _ => false
        ...
        var direction = offsetWall match {
          case Some(xy) if (xy.y == 1 || xy.y == -1) && (xy.x == 1 || xy.x == -1)  => XY.fromDirection45(Random.nextInt(8))
          case _  => {
            offsetBadAss match {
              case Some(xy) if (xy.y == scareLevel || xy.y == -scareLevel) && (xy.x == scareLevel|| xy.x == -scareLevel) => xy.negate
              case _ => {

            offsetPlant match {
                case Some(xy) if (xy.y < 3 && xy.y > -3)||(xy.x < 3 && xy.x > -3) => xy
                case _ => {
                  offsetBeast match {
                  case Some(xy) => xy
                  case _ => XY.fromDirection45(Random.nextInt(8))
                  }
                }
            }
            }
        ...
        if(paramMap.contains("collision")){
          ...
          direction = paramMap.get("collision") match {
            case Some(xyString)  => XY(xyString).negate
          }
        }
        ...
        if(generation ==0 ) {
          ...
          //bot.spawn(XY(-1,25))
          //bot.view.
          //bot.status("Im tha master").toString
          ...
          if(slavemode){
            val slavepos = offsetBadAss match {
              case Some(xy) => xy
              case _ => {
                offsetEnemy match {
                  case Some(xy) => xy
            ...
            bot.move(direction).toString
        }
        else {
          val superoffset = offsetBadAss match {
            case Some(xy) => xy
            case _ => {
              offsetEnemy match {
                case Some(xy) => xy
          ...
          val radius = 4
          if(superoffset.x < radius && superoffset.x > -radius && superoffset.y > -radius && superoffset.y < radius)bot.explode(4)
          bot.move(superoffset).toString // Returns 'Status(text=Hello)'
