package scalatron.botwar.botPlugin.ivar
  ...
  def respond(input: String): String = {
    ...
    opcode match {
      case "React" => {
        ...
        def randomXY = {
          (new scala.util.Random).nextInt(10) match {
            case 0 => XY.Up
            case 1 => XY.RightUp
            case 2 => XY.DownRight
            case 3 => XY.Right
        ...
        var me = bot.view.offsetToNearest(Cell.You)
        var wall = bot.view.offsetToNearest(Cell.Wall)
        var enemy = bot.view.offsetToNearest(Cell.Enemy)
        var plant = bot.view.offsetToNearest(Cell.GoodPlant)
        var badPlant = bot.view.offsetToNearest(Cell.BadPlant)
        var goodBeast = bot.view.offsetToNearest(Cell.GoodBeast)
        var badBeast = bot.view.offsetToNearest(Cell.BadBeast)
        var enemySlave = bot.view.offsetToNearest(Cell.EnemySlave)

        var whatTodo = (me, wall, enemy, plant, badPlant)


        whatTodo match {
          case (Some(me), None, None, Some(plant), Some(badPlant)) => {
            if(me.distanceTo(badPlant) < 2) bot.move(badPlant.negate)
            else bot.move(plant)
          }

          case (Some(me), None, None, Some(plant), None) => {
              bot.move(plant)
          }
          case (Some(me), None, Some(enemy), None, Some(badPlant)) => {
            bot.move(randomXY)
            bot.explode(100)
          }

          case (Some(me), Some(wall), None, Some(plant), Some(badPlant)) => {
            if(me.distanceTo(wall) < 2) bot.move(wall.negate)
            else if(me.distanceTo(wall) < 3) bot.move(randomXY)
            else bot.move(plant)
          }

          case (Some(me), Some(wall), Some(enemy), Some(plant), Some(badPlant)) => {
            if(me.distanceTo(enemy) < 2) {
              bot.move(randomXY)
              bot.say("Explode!!!!")
              bot.explode(1000)
            } else if(me.distanceTo(wall) < 2)  {
              bot.move(randomXY)
            } else {
              bot.move(plant)
            }
          ...
          case (Some(me), Some(wall), _, _, _) => {
              bot.move(randomXY)
          }

          case _ => bot.move(XY.Up)
        ...
        */
        ...
        bot.status("#").toString // Returns 'Status(text=Hello)'
      }
      case _ => ""
    }
  }
