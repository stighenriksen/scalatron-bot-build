package scalatron.botwar.botPlugin.mha
import util.Random
...
class ControlFunction {
  ...
  def respond(input: String): String = {
    ...
    if(opcode=="React") {
      ...
      var shoot = "";
      if (generation == 0) {
        if (paramMap("energy").toInt >= 100 && rnd.nextDouble() < 0.20 ) {
          val heading = XY.random(rnd)
          shoot = "|Spawn(direction=" + heading + ",energy=100," + "heading="+heading+")"
        } else {
      ...
      view.offsetToNearest(Cell.apply('P')) match {
        case Some(offset) =>
          val unitOffset = offset.signum
          "Move(direction=" + unitOffset + ")|Status(text=mha)" + shoot
        case None =>
          ""
      }
      ...
      if (generation == 0) {
        ...
        "Move(direction=" + heading + ":" + heading + ")"
      }
    } else {
      ""
      //"Status(text=marte)"
