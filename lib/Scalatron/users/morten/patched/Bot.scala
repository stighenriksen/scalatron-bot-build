package scalatron.botwar.botPlugin.morten
import java.util.Random
import scalatron.botwar.Command.{Explode, Say}
  ...
  def respond(input: String): String = {
    ...
    opcode match {
      ...
      case "React" => {
        ...
        val goodBeast: Option[XY] = bot.view.offsetToNearest(Cell.GoodPlant)
        val evilBe: Option[XY] = bot.view.offsetToNearest(Cell.BadBeast)

        val rnd = new Random()
        val dx = rnd.nextInt(3) - 1
        val dy = rnd.nextInt(3) - 1

        if (params.contains("role") && n >= 10) {
          Explode.toString()
        } else if (params.contains("role")) {
                ...
                case None =>
                  evilBe match {
                    case Some(offset) =>
                      ...
                      if (params("energy").toInt >= 100) {
                        val direction = offset.signum
                        "Spawn(direction=" + direction + ",energy=100,heading=" + direction + ",role=missile)"
                      } else {
        ...
        //  nearestPlant match {
        //    case Some(XY) => bot.move(nearestPlant.get).toString
        //     case _ =>
        // }
