package scalatron.botwar.botPlugin.martin
    ...
    opcode match {
      case "React" => {
        bot.view.offsetToNearest(Cell.GoodPlant) match {
          case Some(offset) => bot.move(offset.signum).toString
          case None => bot.move(XY.random(rnd)).toString
        }
      }
      case "Welcome" => {
        bot.status("Hello!").toString
      }
      case _ => ""
    }
  }
