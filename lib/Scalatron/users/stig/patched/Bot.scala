package scalatron.botwar.botPlugin.stig
...
//  def create = new HelloBot().respond _
  def create = new MyBot().respond _
}
    ...
    */
    opcode match {
      case "React" => {
        ...
        bot.status("Hello!").toString // Returns 'Status(text=Hello)'
      }
      case _ => ""
    }
  }
...
class MyBot {
    ...
    opcode match {
      case "React" => {
        ...
        if (generation == 0) { // Master bot
          ...
          (nearestPlant, nearestEnemy, bot.energy) match {
            case (_, Some(enemy), e) if e >= 100 =>
              shoot(bot, offset = enemy, target = enemy, detonationTimer = 5).say("geronimo").toString
            case (Some(goodPlant), _, _)  => bot.move(goodPlant).toString
          case _ => bot.move(XY.Up).toString
          }
  ...
  def handleMiniBot(bot: BotImpl): String = {
    val target = bot.inputAsXYOrElse("target", XY.Up)
    val timer = bot.inputAsIntOrElse("timer", 0)
    if (timer == 0) {
      bot.explode(5).toString
    } else {
      bot.move(target).set("timer" -> (timer - 1)).toString
    }
  }

  def shoot(bot: BotImpl, offset: XY, target: XY, detonationTimer: Int) = {
    bot.spawn(offset, "target" -> target, "timer" -> detonationTimer)
  }
}
