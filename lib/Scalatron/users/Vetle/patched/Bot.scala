package scalatron.botwar.botPlugin.Vetle
...
class HelloBot {
  ...
  val statuses = Stream.continually(List(
    //"_*A+<:>",
    //"Scala[-Confusing]",
    "Leeeeeeeeroy Jenkiiiiiins!"
  ).toStream).flatten

  def nextStatus: String = {
    ...
    */
    ...
    opcode match {
      case "React" => {
        ...
        //List(moveCommand(bot), statusCommand(bot)).mkString("|")
        val cmd = moveCommand(bot)

        //List(cmd, Log("foo"), statusCommand(bot)).mkString("|")
        bot.status(nextStatus.toString)
        bot.toString
      }
      case _ => ""
    }
  ...
  val directions = List(Direction45.Down,
    Direction45.DownRight,
    Direction45.Right,
    Direction45.RightUp,
    Direction45.Up,
    Direction45.UpLeft,
    Direction45.Left,
    Direction45.LeftDown)


  def moveCommand(bot: Bot) {
         /*
    val xyOpt = Option(bot.view.offsetsTo(Cell.GoodPlant)/*.filterNot(x => walls.contains(x))*/.head)

    xyOpt match {
      case Some(xy) => bot.move(xy)
      case _ => bot.move(XY.fromDirection45(Random.shuffle(directions).head))
    }
    */
    bot.view.offsetToNearest(Cell.GoodPlant) match {
      case Some(xy) if bot.view.cellAtAbsPos(xy) != 'W' => bot.move(xy)
      case _ => bot.move(XY.fromDirection45(Random.shuffle(directions).head))
    }
  }
...
case class View(cells: String) {
  ...
  def offsetsTo(cell: Cell) = {

    val matchingXY = cells.view.zipWithIndex.filter(_._1 == cell.char)
    if (matchingXY.isEmpty)
      List()
    else {
      val offsets = matchingXY.map(p => relPosFromIndex(p._2)).sortBy(_.length)
      offsets
    }
  }
}
