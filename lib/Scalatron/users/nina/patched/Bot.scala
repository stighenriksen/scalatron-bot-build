package scalatron.botwar.botPlugin.nina
...
class HelloBot {
  ...
  def respond(input: String): String = {
    ...
    val bot = new BotImpl(paramMap)
    val generation = bot.generation
    val energy = bot.energy
    if( opcode == "React" ) {
      val view = View(paramMap("view"))
      view.offsetToNearest(Cell.GoodPlant) match {
        case Some(offset) =>
          val unitOffset = offset.signum
          bot.move(unitOffset).toString
        case None =>
          view.offsetToNearest(Cell.BadBeast) match {
            case Some(offset) =>
              if (bot.generation > 0) {
                bot.explode(5).toString
              } else {
                view.offsetToNearest(Cell.GoodBeast) match {
                  case Some(off) => bot.move(off.signum).toString
                  case None => {
                    val rnd = new Random()
            ...
            case None =>
              ...
              val dy = rnd.nextInt(3) - 1
              val dir = XY(dx, dy)
              if (generation <= 10 && energy >= 100) {
                bot.spawn(XY(dx, dy), "energy"->100, "heading"->dir).toString
              } else bot.move(XY(dx, dy)).toString
          }
      }
    } else ""
  }
