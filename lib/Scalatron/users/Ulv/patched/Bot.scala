package scalatron.botwar.botPlugin.Ulv
...
class ControlFunctionFactory {
  ...
  def create = new WolfBot().respond _
}
...
class WolfBot {
  ...
  def respond(input: String): String = {
    ...
    opcode match {
      case "React" => {
        ...
        //bot.status(bot.generation.toString).toString // Returns 'Status(text=Hello)'

        val targets: Seq[Cell] = whoToChase(bot)

        var result: String = ""
        if(collision(bot)) {
          result = moveAwayFromCollision(bot)
        } else {
          result = moveTowards(bot, targets.headOption.getOrElse(Cell.GoodPlant))
        }

        result = bot.set("lastPos", bot.offsetToMaster).toString
  ...
  def collision(bot: BotImpl): Boolean = {
    bot.inputParams.get("collision") match {
      case Some(_) => true
      case _ => false
    }
  }

  def enemyNear(bot: BotImpl): Boolean = {false}

  def moveAwayFromCollision(bot: BotImpl): String = {
    val dir = bot.inputAsXYOrElse("collision", XY(1,1)).rotateClockwise90

    bot.move(dir).toString
  }

  def moveTowards(bot: BotImpl, cell: Cell): String = {
    bot.move(bot.view.offsetToNearest(cell) match {
      case Some(a) => a
      case None => XY(0, 0)
    }).toString
  }

  def lastPos(bot: BotImpl) = {
    bot.inputAsXYOrElse("lastPos", XY(1,1))
  }

  def whoToChase(bot: BotImpl): Seq[Cell] = {
    val distToGoodPlant = distanceTo(bot, Cell.GoodPlant)
    val distToGoodBeast = distanceTo(bot, Cell.GoodBeast)

    if(distToGoodBeast <= distToGoodPlant-10) {
      Seq(Cell.GoodBeast, Cell.GoodPlant)
    } else {
      Seq(Cell.GoodPlant, Cell.GoodBeast)
    }
  }
  
  def distanceTo(bot: BotImpl, cell: Cell) = {
    bot.view.offsetToNearest(cell) match {
      case Some(a) => bot.offsetToMaster.distanceTo(a)
      case None => 9999999.0
    }
  }
