package scalatron.botwar.botPlugin.kat
import scalatron.botwar.Command.Log
import util.Random
    ...
    */
    ...
    opcode match {
      case "React" => {
        ...
        val dx = rnd.nextInt(5) - 1
        val dy = rnd.nextInt(3) - 1
        val viewString = params("view")
        val view = View(viewString)

        val count = view.offsetToNearest(Cell.GoodPlant).getOrElse(new XY(1,1)).stepCount
        val count2 = view.offsetToNearest(Cell.BadBeast).getOrElse(new XY(0,0))

        view.offsetToNearest(Cell.GoodPlant) match {
          case Some(offset) =>
            val unitOffset = offset.signum
            bot.say("gimme FOOD").move(unitOffset).toString
          case None =>
            view.offsetToNearest(Cell.BadBeast) match {
              case Some(offset) =>
                val unitOffset: XY = offset.signum
                bot.say("a monster!").move(unitOffset.negate).toString
              case None =>
                ""
            }
        }
