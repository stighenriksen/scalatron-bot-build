package scalatron.botwar.botPlugin.jari
...
class HelloBot {
  var lastPos: XY = XY(0, 0)

  def respond(input: String): String = {
    val (opcode, params) = CommandParser(input)

    opcode match {
      case "React" => react(BotImpl(params))
      case _ => ""
    }
  }

  def react(bot: BotImpl): String = {
    val direction = bot.view.offsetToNearest(Cell.GoodPlant)
                            .getOrElse(bot.view.offsetToNearest(Cell.GoodBeast)
                            .getOrElse(randomDirection))

    bot.move(direction).status("d: " + bot.view.cellAtRelPos(direction)).toString
  }

  def randomDirection: XY = new XY(Random.nextInt(5) - 5, Random.nextInt(5) - 5)
}
