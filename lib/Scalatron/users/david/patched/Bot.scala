package scalatron.botwar.botPlugin.david
...
//            bot.status("david").toString // Returns 'Status(text=Hello)'
            val dx = rnd.nextInt(3) - 1
            val dy = rnd.nextInt(3) - 1
            "Move(direction=" + dx * 2 + ":" + dy + ")" +
              "|Spawn(direction=" + dy + ":" + dx + ",energy=100,heading=" + dx + ",role=missile)"
          case _ =>
            val heading = params("heading")
