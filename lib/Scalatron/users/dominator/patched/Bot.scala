package scalatron.botwar.botPlugin.dominator
...
class HelloBot {
  def respond(input: String): String = {
    ...
    */
    opcode match {
      case "React" => {
        params.get("generation") match {
          case Some("0") => {
            val bot: Bot = BotImpl(params)

            if (bot.energy >= 200 && Random.nextInt(100) <= 5) {
              bot.spawn(XY.fromDirection45(Random.nextInt(8)))
            }
            val directionWeCanSafelyGo = findGoodDirection(bot)

            bot.move(directionWeCanSafelyGo)
            bot.status("Dominating")
            bot.set(("heading",directionWeCanSafelyGo.signum)).toString
          }
          case _ => {
            val miniBot: MiniBot = BotImpl(params)

            val directionWeCanSafelyGo = if (miniBot.energy > 2000) {
              miniBot.offsetToMaster
            } else if (miniBot.energy > 500 && miniBot.offsetToMaster.stepCount < 10) {
              miniBot.offsetToMaster
            } else {
              findGoodDirection(miniBot)
            }

            miniBot.move(directionWeCanSafelyGo)
            miniBot.status("mini dom!")
            miniBot.set(("heading",directionWeCanSafelyGo.signum)).toString
          }
        }
      }
      case _ => ""
  ...
  def findGoodDirection(bot: Bot) = {
    val closestGoodBeast = bot.view.offsetToNearest(Cell.GoodBeast)
    val closestGoodPlant = bot.view.offsetToNearest(Cell.GoodPlant)

    val placeWeWantToGo = (closestGoodBeast, closestGoodPlant) match {
      case (Some(beast), Some(plant)) if bot.view.center.stepsTo(beast) + 2 < bot.view.center.stepsTo(plant) => {
        beast
      }
      case (Some(beast), None) => {
        beast
      }
      case (_, Some(plant)) => {
        plant
      ...
      case _ => {
        if (Random.nextInt(100)<10)
          ...
          bot.inputAsXYOrElse("heading", XY.fromDirection45(Random.nextInt(8)))
      }
    }

    safify(bot.view, placeWeWantToGo)
  }

  def safify(view: View, place: XY): XY =
    safify(view, place, place.signum)

  def safify(view: View, place: XY, direction: XY): XY = {
    if (badCells.contains(Cell(view.cellAtRelPos(direction.signum))))
      safify(view, place, findDesiredDirection(view, place, direction.signum))
    else
      direction.signum
  }

  def findDesiredDirection(view: View, place: XY, direction: XY): XY = {
    val surroundings = Seq(
      view.center + direction,
      view.center + direction.rotateClockwise45,
      view.center + direction.rotateClockwise90,
      view.center + direction.rotateClockwise90.rotateClockwise45,
      view.center + direction.rotateClockwise90.rotateClockwise90,
      view.center + direction.rotateClockwise90.rotateClockwise90.rotateClockwise45,
      view.center + direction.rotateClockwise90.rotateClockwise90.rotateClockwise90,
      view.center + direction.rotateClockwise90.rotateClockwise90.rotateClockwise90.rotateClockwise45
    )
    val safeCells = surroundings.filterNot(x=>badCells.contains(Cell(view.cellAtAbsPos(x))))
    val safeCellsByDistanceToGoal = safeCells.sortBy{_.distanceTo(view.absPosFromRelPos(place))}
    safeCellsByDistanceToGoal.head - view.center
  }

  val badCells: Seq[Cell] = Seq(Cell.Wall, Cell.BadBeast, Cell.BadPlant, Cell.EnemySlave, Cell.Enemy)
...
trait Bot {
  ...
  def inputAsXYOption(key: String): Option[XY]
  def view: View
  def energy: Int
  def time: Int
  def generation: Int
...
case class BotImpl(inputParams: Map[String, String]) extends MiniBot {
  ...
  def inputAsXYOption(key: String) = inputParams.get(key).map(s => XY(s))
