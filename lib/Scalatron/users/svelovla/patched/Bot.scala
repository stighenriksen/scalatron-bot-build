package scalatron.botwar.botPlugin.svelovla
...
class ControlFunctionFactory {
  ...
  def create = new Lobot().respond _
}
    ...
    */
    opcode match {
      case "React" => {
        ...
        val nearestGoodPlant: Option[XY] = bot.view.offsetToNearest(Cell.GoodPlant)
        val nearestGoodBeast: Option[XY] = bot.view.offsetToNearest(Cell.GoodBeast)
        val nearestWall: Option[XY] = bot.view.offsetToNearest(Cell.Wall)
        val nearestbadBeast: Option[XY] = bot.view.offsetToNearest(Cell.BadBeast)
        val nearestSlave: Option[XY] = bot.view.offsetToNearest(Cell.YourSlave)

        if (nearestbadBeast.isDefined && !nearestSlave.isDefined){
          val beastXy = nearestbadBeast.get
          bot.spawn(beastXy)
          bot.say("Lobo spawned")
          bot.move(beastXy)

          if (XY(0,0).distanceTo(beastXy) < 100.0){
            bot.say("Pew pew")
            bot.explode(100)
          }
        }
        ...
        bot.move(nearestGoodPlant.getOrElse(nearestGoodBeast.getOrElse(nearestWall.map(_.negate).getOrElse(XY(1,0)))))


        //nearestbadBeast.map(xy: XY => Unit = bot.spawn(xy) )
        //bot.move(XY(10,10))

        bot.toString // Returns 'Status(text=Hello)'
      }
      case _ => ""
    }
  }
