package scalatron.botwar.botPlugin.daria
import java.util.Random
import scalatron.botwar.Command.Move
    ...
    */
    opcode match {
      case "React" => {
        ...
        bot.status("Privet!").toString // Returns 'Status(text=Hello)'
      }
      case _ => ""
    }
  }
...
/*        val generation = paramMap("generation").toInt
        ...
        val xy: XY = view.center
        xy.addToX(1)
        xy.addToY(2)
        bot.move(xy).say("I'm moving").toString
